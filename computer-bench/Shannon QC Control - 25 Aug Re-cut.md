...
**What counts as a run:** a trial that produced deliverables and was graded by the verifier. Agent-broken attempts — **genuine incompletion only**: the agent read and worked, the transcript is complete, and it failed to produce output (e.g. step-length death; see playbook §11.5) — count as neither pass nor fail; document, replace (see below). Network disconnects and kills are a **different evidence state**, not the same class: died in setup/preflight → known no-work, replace, uncounted; died mid-agent → **unknown completion** (the agent may have completed just fine before the record stops) — never classify it as "genuinely couldn't complete", never count it as a pass; inspect the trajectory (deliverables present → recover and grade, flagged "trajectory-recovered"; absent → rerun and document as unknown). All broken/unknown attempts are listed as exceptions (attempt, reason, recovered-or-not) and are never evidence about task difficulty. One floor regardless of the exceptions: **the loop only ends once ≥1 run passes** — the solvability gate needs a reward-1.0 non-oracle run, and with zero passing runs there is no difficulty evidence either way.
