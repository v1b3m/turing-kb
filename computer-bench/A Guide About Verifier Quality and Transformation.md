...
A trainer-facing guide to writing and fixing verifiers: the seven ways verifiers most commonly break, how to recognize each one in a real manifest, what the fix looks like, and a ready-to-use prompt for transforming a broken verifier suite into a fair one. Read this alongside **Layer 5 · Verifier coverage and fairness** in the [*ComputerBench QC Review Guide*](https://docs.google.com/document/d/1KVqG9gSf4oRIsY3rGoxcfWred-Z4Gj1_wCl9o7U5SWM/edit?usp=drive_link) — this document is the "how," that section is the "what to check."

> 🛠️ [**lean\_prompt\_transforming\_verifiers.md**](https://drive.google.com/file/d/1cxIwKQg6Rw8lzqVWEbuGaP4qhJ4SvoVd/view?usp=drive_link) — the ready-to-run prompt for transforming a task's verifiers. Use this instead of writing your own prompt from scratch.

---

## 1\. What a high-quality verifier establishes

Before touching any verifier, know what it has to prove. A high-quality verifier establishes that:
...
Every fix below exists to restore one of these eight properties.

---

## 2\. The failure taxonomy — 7 ways verifiers break

Nearly all verifier feedback maps to one of these, ordered by how often it shows up and how much damage it does:
...
Failure mode 1 alone accounts for roughly half of all verifier defects — start there on every task.

---

## 3\. Failure mode 1 — Brittle regex and exact-string grading *(the dominant problem)*

**Typical false negatives you'll see:**
...
**Anti-pattern** — fragile, exact prose, unanchored, negation-blind:
...
**Better** — grade the fact structurally from `results.json`:
...
```json
{
  ...
  "assertion": { "type": "deterministic", "expected": 3,
                 "deterministic": { "path": "$.invalid_codes",
                                    "comparison": "equals" } }
}
```

If you must match prose, anchor and lengthen the stem:
...
**Mismatch** — instruction says "write the report to `report.md`," but the check grades the final chat message:

```json
"source": { "target": "final_answer" }        // wrong target
```
...
"source": { "type": "file", "file": {
   "type": "md", "command": "extract_text",
   "arguments": { "path": "report.md" } } }
```

Or change the instruction to explicitly require the figures in chat. Pick one — don't let the two drift.
...
Reward is 1.0 only if every check passes, so a single defective check zeroes a materially correct run. Most "63/64 passed, reward 0.0" cases are a defective check, not an incomplete answer.

**Rules:**

- Fix the underlying brittle/mismatched checks first — that removes most of these failures.  
...
- Keep the behavioral gate small and structured (a few derived figures in `results.json`); make secondary prose/format checks non-fatal or semantic.
...
**Instead** — execute the real suite against the model's code, and grade the output it produces (`results.json`), not a self-report.
...
| A money / ratio figure | `approx_equals` with a tolerance | exact `equals` on a float |
...
When you're staring at a check and unsure how to grade it, find its row here first.

---

## 10\. Reward-hacking review

Adversarially review every high-weight verifier. For each one, ask: **what's the cheapest way to pass without doing the work?** Confirm the agent cannot:
...
## 12\. The three-question test — before any check ships

Every verifier check must answer all three, or it doesn't ship:

**A. What requirement is this checking?** The reviewer must be able to point to the relevant instruction or task specification.

**B. Why is this check sufficient?** Explain why passing the check establishes the requirement.

**C. Why is this check appropriately permissive?** Explain why a valid alternative solution would not be rejected merely because it uses different wording, ordering, formatting, aliases, or an equivalent implementation.

A verifier check without a clear answer to all three questions should not ship.

---

## 13\. How to actually transform a verifier suite

1. **Read the manifest against the instruction**, checking each of the 7 failure modes above in order of §2's frequency — start with brittle regex, then mismatch, then the rest.  
2. **Use the ready-made prompt** — [lean\_prompt\_transforming\_verifiers.md](https://drive.google.com/file/d/1cxIwKQg6Rw8lzqVWEbuGaP4qhJ4SvoVd/view?usp=drive_link) — to have an LLM rewrite the verifier suite against these same rules. It's the packaged, lean version of the prompt template below.  
3. **Never accept the output blindly.** Review every changed check against the decision framework (§9) and the three-question test (§12) before it ships. The LLM can make mistakes — hardcoding a value, deleting a real correctness check, or inventing a requirement that isn't in the instruction.  
4. **Re-run the Oracle** after any verifier change — it must still score exactly 1.0.  
5. **Run the reward-hacking review (§10)** on every high-weight check you touched.

### The underlying prompt template

If you need to write your own transformation prompt (or want to see what the lean prompt above is built on):
...
- [**Verifier Quality Guide**](https://docs.google.com/document/d/1vr-ziO1H_RU2Q13F4wK9n01VLaTK8DY8/edit?usp=sharing) — the failure taxonomy, decision framework, reward-hacking checklist, and transformation prompt this guide is built from.  
- [**lean\_prompt\_transforming\_verifiers.md**](https://drive.google.com/file/d/1cxIwKQg6Rw8lzqVWEbuGaP4qhJ4SvoVd/view?usp=drive_link) — the ready-to-run prompt for transforming a task's verifiers.
